...
<div class="<?php echo cs_allow_special_char($main_col); ?>">
    <div class="cs-jobs-container">
        <div class="hiring-holder custom-styling-list recent-jobs">
            <?php
            ...
            if ((isset($a['cs_job_title']) && $a['cs_job_title'] != '') || (isset($a['cs_job_top_search']) && $a['cs_job_top_search'] != "None" && $a['cs_job_top_search'] <> '')) {
                ...
                if (isset($a['cs_job_title']) && $a['cs_job_title'] != '') {
                    ...
                    if (isset($count_post) && $count_post != '') {
                        echo '<strong class="recent-results"> ( ' . $count_post . ' ' . esc_html__('Jobs', 'jobhunt') . ' )</strong>';
                    }
                    echo '</div>';
                }
            ...
            <ul class="row jobs-listing joblist-boxed">
                <?php
                ...
                if ($loop->have_posts()) {
                    ...
                    while ($loop->have_posts()) : $loop->the_post();
                        ...
                        $job_address = apply_filters( 'jobhunt_job_address_frontend', $job_address, $cs_job_id);
                        ?>
                        <li class="col-lg-6 col-md-6 col-xs-12 col-xs-12">
                                ...
                                <div class="jobs-holder">
                                    ...
                                    <div class="cs-text">
                                        ...
                                        <div class="cs-post-title">
                                            <h6>
                                                <a href="<?php echo esc_url(get_permalink($cs_job_id)); ?>"><?php echo esc_html(get_the_title($cs_job_id)); ?></a>
                                            </h6>
                                        </div>
                                        <?php
                                    ...
                                    <div class="options">
                                        ...
                                        if (!is_user_logged_in() || !$login_user_is_employer_flag) {
                                            ...
                                            if (is_user_logged_in()) {
                                                ...
                                                ?>
                                                <button type="button" class="heart-btn shortlist" data-toggle="tooltip" data-placement="top" title="<?php esc_html_e("Add to Shortlist", "jobhunt"); ?>" onclick="trigger_func('#btn-header-main-login');"><i class='icon-star-o'></i></button>
                                                <?php
                                            }
                                        } else {
                                            ?>
                                            <a class="whishlist_icon shortlist" href="javascript:void(0)"  data-toggle="tooltip" data-placement="top" title="<?php esc_html_e("Add to Shortlist", "jobhunt"); ?>" id="<?php echo 'addjobs_to_wishlist' . intval($cs_job_id); ?>" onclick="show_alert_msg('<?php echo esc_html__("Become a candidate then try again.", "jobhunt"); ?>')" ><i class="icon-star-o"></i></a>
                                            <?php
                                        }
                                        ?>

                                        <a href="<?php echo esc_url(get_permalink($cs_job_id)) ?>">
                                            <i class="icon-reply4"></i>
                                        </a>
                                    </div>
                                </div>
                            </div>
