...
if (!function_exists('cs_google_uniqid')) {
    ...
    function cs_google_uniqid() {
        ...
        setcookie('cs_google_uniqid', $_COOKIE['cs_google_uniqid'], current_time('timestamp') + 3600, '/');
        ...
        if ($client->getAccessToken()) {
            ...
            try {
                ...
                if (!is_user_logged_in()) {
                    ...
                    set_site_transient($user_info->ID . '_cs_google_admin_notice', esc_html__('This Google profile is already linked with other account. Linking process failed!', 'jobhunt'), 3600);
                }
            }
            catch (Google_ServiceException $e) {
                echo sprintf('<p>' . __( 'A service error occurred:', 'jobhunt' ) . ' <code>%s</code></p>', htmlspecialchars($e->getMessage()));
                exit;
            } catch (Google_Exception $e) {
                echo sprintf('<p>' . __( 'A client error occurred:', 'jobhunt' ) . ' <code>%s</code></p>', htmlspecialchars($e->getMessage()));
                exit;
            } catch (apiServiceException $e) {
                // Handle exception. You can also catch Exception here.
                // You can also get the error code from $e->getCode();

                echo __('Error Code', 'jobhunt') . ': ' . $e->getCode() . '<br/>';
                echo __('Authentication failed due to Invalid Credentials ', 'jobhunt');
                exit;
            }
        } else {
...
if (!function_exists('cs_google_login_url')) {
    ...
    function cs_google_login_url() {
        //global $post;
        return site_url('wp-login.php', 'admin') . '?loginGoogle=1';
    }
}
