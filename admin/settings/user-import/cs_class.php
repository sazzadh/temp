    ...
    public function get_row() {
        ...
        while (1) {
            ...
            if ($state == self::quoted_field) {
                ...
                // Possible start of  line end, but might be just part of foo\rbar
            ...
            switch ($state) {
                ...
                case self::found_cr:
                    // Lone \rX instead of . Treat as literal \rX. (TODO: error case?)
                    $field .= "\r" . $char;
                    $state = self::unquoted_field;
                    break;

                case self::found_cr_q:
                    // (TODO: error case: "foo"\rX instead of "foo" or "foo"\n)
                    $field .= "\r" . $char;
                    $state = self::quoted_field;
                    break;
            }
