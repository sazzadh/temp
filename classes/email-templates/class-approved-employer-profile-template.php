 ...
 */
if ( ! class_exists('jobhunt_approved_employer_profile_template') ) {

    class jobhunt_approved_employer_profile_template {
        ...
        function get_approved_employer_user_name() {
            $employer_info = get_user_by('id', $this->employer_id);
            return $employer_info->display_name;
        }

        function get_approved_employer_user_email() {
            $employer_info = get_user_by('id', $this->employer_id);
        ...
        public function employer_profile_status_changed($employer_id, $employer_old_status) {
            ...
            if ( $employer_id != '' ) {
                ...
                if ( $role == 'cs_employer' ) {
                    ...
                    // checking job status
                    if ( $employer_status == 'active' && $employer_status != $employer_old_status ) {
                        ...
                        if ( isset($template['email_notification']) && $template['email_notification'] == 1 ) {
                            ...
                            $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : __('Approved Employer Profile', 'jobhunt');
                            $from = (isset($template['from']) && $template['from'] != '') ? $template['from'] : esc_attr($blogname) . ' <' . $admin_email . '>';
                            $recipients = (isset($template['recipients']) && $template['recipients'] != '') ? $template['recipients'] : $this->get_approved_employer_user_email();
        ...
        public function add_email_template() {
            ...
            $email_templates[$this->template_group][$this->email_template_index] = array(
                ...
                'description' => __('This template is used to sending email when employer Status approved', 'jobhunt'),
                'jh_email_type' => 'html',
            );
    ...
    new jobhunt_approved_employer_profile_template();
}